...
export default function Dashboard({
  ...
  const getVolatilityInterpretation = (
    sentiment: number,
    momentum: number,
    trend: number,
  ): string => {
    ...
    const momentumLabel =
      momentum < 0.35
        ...
        : momentum < 0.55
          ? "neutral price momentum"
          : "upward price momentum";

    const trendLabel =
      trend < 0.35
        ? "sentiment stabilising"
        : trend < 0.55
          ? "flat sentiment trend"
              ...
              >
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="text-right">
                      <p className="text-base font-bold text-white">
                        AU$
                        {audRate
                          ? (asset.latest_price * audRate).toLocaleString(
                              "en-AU",
                              {
                      ...
                      <p className="text-sm text-gray-400">
                        US$
                        {asset.latest_price?.toLocaleString("en-US", {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}
                      </p>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`text-sm font-bold ${LEVEL_TEXT[asset.color]}`}
                    >
                  ...
                  <p className="text-xs text-gray-600 mt-2">
                    Updated:{" "}
                    {new Date(asset.analysed_at).toLocaleString("en-AU", {
                      day: "2-digit",
                      month: "2-digit",
                      year: "numeric",
                      hour: "2-digit",
                  ...
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="text-right">
                      <p className="text-xl font-bold text-white">
                        {asset.latest_price?.toLocaleString("en-AU", {
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="text-right">
                  ...
                  <div className="flex items-center gap-1 justify-end mt-1">
                    {audInrVolatility &&
                      getMomentumIcon(audInrVolatility.momentum_score)}
                    <span className="text-xs text-gray-400">momentum</span>
                  </div>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="w-32 bg-gray-800 rounded-full h-1.5">
                    <div
                      ...
                      style={{
                        width: `${audInrVolatility ? audInrVolatility.volatility_score * 100 : 0}%`,
                      }}
                    />
                  </div>
                </div>
                <span
                  className={`text-sm font-bold ${audInrVolatility ? LEVEL_TEXT[audInrVolatility.color] : "text-gray-400"}`}
                >
              ...
              <p className="text-xs text-gray-500 mt-1.5 italic">
                {audInrVolatility
                  ? getVolatilityInterpretation(
                      audInrVolatility.sentiment_score,
                      audInrVolatility.momentum_score,
                      audInrVolatility.trend_score,
                    )
                  : "—"}
              </p>
              <p className="text-xs text-gray-600 mt-2">
                {audInrVolatility?.calculated_at
                  ? `Updated: ${new Date(
                      audInrVolatility.calculated_at,
